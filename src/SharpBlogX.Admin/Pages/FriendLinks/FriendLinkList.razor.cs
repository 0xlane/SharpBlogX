...
namespace SharpBlogX.Admin.Pages.FriendLinks
{
    public partial class FriendLinkList
    {
        ...
        {
            var response = await GetResultAsync<BlogResponse<List<GetAdminFriendLinkDto>>>("api/blog/admin/friendlinks");
            return response.Result;
        }

        public async Task DeleteAsync(string id)
        {
            var response = await GetResultAsync<BlogResponse>($"api/blog/friendlink/{id}", method: HttpMethod.Delete);
            if (response.Success)
            {
        ...
        public async Task HandleSubmit()
        {
            ...
            var response = await GetResultAsync<BlogResponse>($"api/blog/friendlink/{friendlinkId}", json, HttpMethod.Put);
            if (response.Success)
            {
                Close();
