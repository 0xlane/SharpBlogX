...
namespace SharpBlogX.Admin.Pages.Posts
{
    public partial class PostList
    {
        ...
        {
            var response = await GetResultAsync<BlogResponse<PagedList<GetAdminPostDto>>>($"api/blog/admin/posts/{page}/{limit}");
        ...
        {
            var response = await GetResultAsync<BlogResponse>($"api/blog/post/{id}", method: HttpMethod.Delete);
            if (response.Success)
            {
