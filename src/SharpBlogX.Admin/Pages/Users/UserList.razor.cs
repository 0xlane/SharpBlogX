...
namespace SharpBlogX.Admin.Pages.Users
{
    public partial class UserList
    {
        ...
        {
            var response = await GetResultAsync<BlogResponse<List<UserDto>>>("api/users");
            return response.Result;
        }

        public async Task ChangeIsAdminAsync(string id, bool isAdmin)
        {
            var response = await GetResultAsync<BlogResponse>($"api/user/{id}/{isAdmin}", method: HttpMethod.Put);
            if (response.Success)
            {
        ...
        {
            var response = await GetResultAsync<BlogResponse>($"api/user/{id}", method: HttpMethod.Delete);
            if (response.Success)
            {
                await Message.Success("Successful", 0.5);
                users = await GetUserListAsync();
        ...
        public async Task HandleSubmit()
        {
            ...
            var response = await GetResultAsync<BlogResponse>($"api/user/{userId}", json, HttpMethod.Put);
            if (response.Success)
            {
                users = await GetUserListAsync();
                Close();
        ...
        {
            var response = await GetResultAsync<BlogResponse>($"/api/user/password/{userId}/{model.Password}", method: HttpMethod.Put);
            if (response.Success)
            {
                ClosePasswordBox();
                await Message.Success("Successful", 0.5);
        ...
        private async Task Open(string id)
        {
            ...
            var response = await GetResultAsync<BlogResponse<UserDto>>($"api/user/{id}");
            if (response.Success)
            {
