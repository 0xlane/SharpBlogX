...
namespace SharpBlogX.Admin.Pages.Tags
{
    public partial class TagList
    {
        ...
        {
            var response = await GetResultAsync<BlogResponse<List<GetAdminTagDto>>>("api/blog/admin/tags");
            return response.Result;
        }

        public async Task DeleteAsync(string id)
        {
            var response = await GetResultAsync<BlogResponse>($"api/blog/tag/{id}", method: HttpMethod.Delete);
            if (response.Success)
            {
        ...
        public async Task HandleSubmit()
        {
            ...
            var response = await GetResultAsync<BlogResponse>($"api/blog/tag/{tagId}", json, HttpMethod.Put);
            if (response.Success)
            {
                Close();
                await Message.Success("Successful", 0.5);
