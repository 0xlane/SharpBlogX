...
namespace SharpBlogX.Admin.Pages.Messages
{
    public partial class MessageList
    {
        ...
        {
            var response = await GetResultAsync<BlogResponse<PagedList<MessageDto>>>($"api/messages/{page}/{limit}");
        ...
        public async Task SubmitMessageAsync()
        {
            ...
            var response = await GetResultAsync<BlogResponse>("api/message", json, HttpMethod.Post);
            if (response.Success)
            {
        ...
        public async Task SubmitReplyMessageAsync()
        {
            ...
            var response = await GetResultAsync<BlogResponse>($"api/message/reply/{MessageId}", json, HttpMethod.Post);
            if (response.Success)
            {
                await Message.Success("Successful", 0.5);
        ...
        {
            var response = await GetResultAsync<BlogResponse>($"api/message/{id}", method: HttpMethod.Delete);
            if (response.Success)
            {
                await Message.Success("Successful", 0.5);
        ...
        {
            var response = await GetResultAsync<BlogResponse>($"api/message/reply/{id}/{replyId}", method: HttpMethod.Delete);
            if (response.Success)
            {
                await Message.Success("Successful", 0.5);
