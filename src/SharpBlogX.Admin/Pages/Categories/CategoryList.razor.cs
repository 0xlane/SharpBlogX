...
namespace SharpBlogX.Admin.Pages.Categories
{
    public partial class CategoryList
    {
        ...
        {
            var response = await GetResultAsync<BlogResponse<List<GetAdminCategoryDto>>>("api/blog/admin/categories");
            return response.Result;
        }

        public async Task DeleteAsync(string id)
        {
            var response = await GetResultAsync<BlogResponse>($"api/blog/category/{id}", method: HttpMethod.Delete);
            if (response.Success)
            {
        ...
        public async Task HandleSubmit()
        {
            ...
            var response = await GetResultAsync<BlogResponse>($"api/blog/category/{categoryId}", json, HttpMethod.Put);
            if (response.Success)
            {
                Close();
                await Message.Success("Successful", 0.5);
