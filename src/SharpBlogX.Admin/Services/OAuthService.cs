...
namespace SharpBlogX.Admin.Services
{
    public class OAuthService : AuthenticationStateProvider
    {
        ...
        public override async Task<AuthenticationState> GetAuthenticationStateAsync()
        {
            ...
            else
            {
                ...
                var httpResponseMessage = await http.GetAsync("api/user");
        ...
        public async Task<string> GetTokenAsync(LoginInput login)
        {
            ...
            if (login.Type == "account")
            {
                ...
                httpResponse = await http.PostAsync("api/oauth/account/token", content);
            }
            else
            {
                httpResponse = await http.PostAsync($"api/oauth/token?code={login.Code}", null);
            }
